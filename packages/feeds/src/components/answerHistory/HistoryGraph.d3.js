import * as d3 from 'components/answerHistory/d3'
import { ethers } from 'components/answerHistory/ethers'
import { formatAnswer } from 'contracts/utils'
import { humanizeUnixTimestamp } from 'components/answerHistory/utils'
...
export default class HistoryGraph {
  ...
  updateBrushed() {
    ...
    if (extent) {
      ...
      this.zoomOutBtn.transition().duration(300).style('opacity', 1)
    }
    ...
    this.path.transition().duration(300).attr('d', line)
  }
  ...
  zoomOut(data) {
    ...
    this.xAxis.transition().duration(300).call(xAxis)
    ...
    this.zoomOutBtn.transition().duration(300).style('opacity', 0)
  }
  ...
  updateMa(data) {
    ...
    this.bollingerMa.datum(bandsData).style('opacity', 0).attr('d', ma)
  }
